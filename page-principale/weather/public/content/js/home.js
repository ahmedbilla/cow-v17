...
// Set default location to Morocco
const DEFAULT_LATITUDE = 31.7917;
const DEFAULT_LONGITUDE = -7.0926;
...
const createWeatherDataBox = (weatherResponse) => {
  ...
  const children_element_of_weather_box = `<h3 class="p-0 m-0"><img src="https://openweathermap.org/img/w/${weather_icon}.png"> ${day}</h3>
  ...
  //* if weather reponse day === today , change the background of this box
  if (day === today) {
...
const createTodayWeatherDataBox = (weather_today_object) => {
  ...
  document.querySelectorAll(
    ".weather-today-temp"
  )[0].innerHTML = `day: ${weather_temp_day}`;
  document.querySelectorAll(
    ".weather-today-temp"
  )[1].innerHTML = `eve: ${weather_temp_eve}`;
  document.querySelectorAll(
    ".weather-today-temp"
  )[2].innerHTML = `night: ${weather_temp_night}`;
  document.querySelector(
    ".weather-today-date"
  ).innerHTML = `<i class="bi bi-calendar2 me-1"></i>  ${createDate()}`;
...
const convertWeatherTodayBoxToError = () => {
  ...
  document
    .querySelectorAll(".weather-today-temp")
...
const getLatAndLonOfCity = (city) => {
  ...
  // If the city is not provided, use the default coordinates for Morocco
  const query = city ? city : "Morocco";

  const API = `https://us1.locationiq.com/v1/search.php?key=${API_KEY}&q=${query}&format=${format}&accept-language=${Accept_Language}&limit=${limit}`;

  let res_address = "";

  //* get request to geocode api
  makeRequest(API, "Error getting geolocation data")
    .then((res) => {
      if (!res || res.length === 0) {
      ...
      return getWeatherResponse(latt, longt, "Error getting weather info");
    })
    .then((res) => {
...
const createErorr = (err = "") => {
  ...
  tryAgainBtn.addEventListener("click", () =>
    getLatAndLonOfCity(searchCityInput.value)
  );
  ...
  requestToRandomCity.addEventListener("click", () =>
